...
@section('pages')
    <section class="min-h-screen">
        ...
        <div class="bg-white shadow rounded px-4 py-8">
            <form action="{{ route('kegiatan-prodi.update', $data->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('put')
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-3 mb-3">
                    ...
                    <x-form.input type="text" name="judul" label='Judul Postingan' :value="old('judul', $data->judul)" />
                    <x-form.input type="text" name="user_id" label='Uploader' :value="strtoupper(auth()->user()->role)" :readonly="true" />
                    <x-form.select name="kategori" label='Kategori' placeholder='Pilih Kategori'>
                        @foreach ($kategori as $r)
                            @if ($r->id == old('kategori', $data->kategori_id))
                                <option value=" {{ $r->id }}" class="capitalize" selected> {{ $r->nama }}
                                </option>
                            @endif
                            <option value=" {{ $r->id }}" class="capitalize"> {{ $r->nama }}</option>
                        @endforeach
                    </x-form.select>
                </div>
                ...
                <div class="my-4 w-full md:w-1/2">
                    <div>
                        <label for="tags-input" class="label-ct">Input Hastag <span class="text-red-400">*</span></label>
                        <div id="tags-input" class="">
                            <input id="tags-input" name="tagsvalue" type="text" placeholder="Masukkan Tag"
                                class="input-ct" value="{{ old('tagsvalue', $data->hashtag) }}" />
                        </div>
                        <small class="tag-label" for="taginput">masukkan tag min 3 karakter, pisahkan dengan koma</small>
                    </div>
                </div>

                <x-form.text-editor type="text" name="deskripsi" label='Isi Postingan' :value="old('deskripsi', $data->deskripsi)"
                    :required="true" />
                <div class="mt-7 flex
                        gap-4 items-center">
                    <x-form.button-back :url="route('kegiatan-prodi.index')" />
