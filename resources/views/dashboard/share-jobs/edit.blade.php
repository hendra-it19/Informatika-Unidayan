    ...
    <section class="min-h-screen">
        ...
        <div class="bg-white shadow rounded px-4 py-8">
            ...
            @if ($data->status == 'tolak')
                <p class="text-sm text-red-500">Di tolak dengan catatan : {{ $data->pesan }}</p>
            @endif

            <form action="{{ route('share-jobs.update', $data->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('put')
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-3">
                    <x-form.input type="text" name="mitra" label='Nama Mitra' :value="old('mitra', $data->mitra)" />
                    <x-form.input type="text" name="pekerjaan" label='Pekerjaan' :value="old('pekerjaan', $data->pekerjaan)" />
                    <x-form.input type="date" name="batas_penerimaan" label='Batas Penerimaan' :value="old('batas_penerimaan', $data->batas_penerimaan)" />
                </div>
                <div class="w-full mt-5">
                    <x-form.text-editor label="Deskripsi Pekerjaan" name="deskripsi" :value="old('deskripsi', $data->deskripsi)" :required="true" />
                </div>
                <div class="mt-7 flex gap-4 items-center">
                    <x-form.button-back :url="route('share-jobs.index')" />
                    @if ($data->status == 'tolak')
                        <x-form.button-submit label="Ajukan Ulang" />
                    @else
