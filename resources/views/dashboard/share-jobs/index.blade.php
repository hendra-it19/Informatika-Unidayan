    ...
    <section class="min-h-screen">
        ...
        <div>
            <div class="w-full flex justify-between items-end">
                <form action="{{ route('share-jobs.index') }}" method="GET">
                    <div
                ...
                <a href="{{ route('share-jobs.create') }}">
                    ...
                    <tbody>
                        @forelse ($data as $r)
                            <tr class="bg-white border-b hover:bg-gray-50 text-xs">
                                ...
                                <td class="p-2">
                                    <a href="{{ route('share-jobs.show', $r->id) }}"
                                        class="text-blue-500 underline">{{ $r->pekerjaan }}</a>
                                </td>
                                <td class="p-2">
                                ...
                                <td class="p-2">
                                    <img src="@if (empty($r->foto)) https://placehold.co/100?text=? @else {{ asset($r->foto) }} @endif"
                                        alt="{{ $r->nim }}"
                                        class="w-[70px] aspect-square object-cover bg-cover rounded">
                                </td>
                                <td class="p-2">
                                    {{ $r->alumni->nama }}
                                </td>
                                ...
                                <td class="p-2">
                                    @if ($r->status == 'pengajuan')
                                        <span class="p-0.5 rounded bg-yellow-400 text-white">Pengajuan</span>
                                    @elseif($r->status == 'konfirmasi')
                                        <span class="p-0.5 rounded bg-primary-400 text-white">Konfirmasi</span>
                                    @elseif ($r->status == 'tolak')
                                        <span class="p-0.5 rounded bg-red-400 text-white">Tolak</span>
                                    @endif
                                </td>
                                <td class="p-2 relative">
                                    ...
                                    <div
                                        class="absolute peer-focus:flex top-0 right-2 bg-white shadow-lg rounded p-2 hover:flex gap-1 hidden">
                                        @if ($r->status == 'konfirmasi')
                                            <a href="javascript:void(0)"
                                                class="inline-block w-[18px] text-yellow-300 cursor-not-allowed">
                                            ...
                                            <a href="javascript:void(0)"
                                                class="inline-block w-[18px] text-red-300 cursor-not-allowed">
                                                <svg class="aspect-square w-full" aria-hidden="true"
                                                    xmlns="http://www.w3.org/2000/svg" fill="none"
                                                    viewBox="0 0 24 24">
                                                    <path stroke="currentColor" stroke-linecap="round"
                                        ...
                                        @else
                                            <a href="{{ route('share-jobs.edit', $r->id) }}"
                                            ...
                                            <form action="{{ route('share-jobs.destroy', $r->id) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
