...
@section('pages')
    <section class="min-h-screen">
        ...
        <div class="bg-white shadow rounded px-4 py-8">
            <form action="{{ route('postingan-organisasi.update', $data->id) }}" method="POST"
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-3 mb-3">
                    ...
                    @if ($role == 'organisasi')
                        <x-form.input type="text" name="organisasi" label='Organisasi' :value="old('organisasi', $data->organisasi->nama_organisasi)"
                            readonly="true" />
                    ...
                    @else
                        <x-form.select name="organisasi" label='Organisasi' placeholder='Pilih organisasi'>
                            @foreach ($organisasi as $r)
                                @if ($r->id == old('organisasi', $data->organisasi_id))
                    ...
                    <div>
                        ...
                        <div id="tags-input">
                            <input id="tags-input" name="hashtag" type="text" placeholder="Masukkan Tag" class="input-ct"
                                value="{{ old('hashtag', $data->hashtag) }}" required />
                            @error('hashtag')
                                <small class="-mt-[1px] block text-red-500">{{ $message }}</small>
                            @enderror
                        </div>
