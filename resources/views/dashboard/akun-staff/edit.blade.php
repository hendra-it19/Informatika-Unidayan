...
@section('pages')
    <section class="min-h-screen">
        ...
        <div class="bg-white shadow rounded px-4 py-8">
            <form action="{{ route('akun-staff.update', $data->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('put')
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-3">
                    ...
                    <x-form.input type="number" name="nomor_identitas" label='Nomor Identitas' :value="old('nomor_identitas', $data->identitas)" />
                    ...
                    <x-form.select name="hak_akses" label='Hak Akses' placeholder='Pilih Hak Akses'>
                        @foreach ($hak_akses as $r)
                            @if ($r['nama'] == old('hak_akses', $data->role))
                    ...
                    <div>
                        <label for="verifikator" class="text-gray-700">verifikator <span
                                class="text-red-400">*</span></label>
                        <div class="text-sm flex gap-3">
                            <div class="@if ($verifikator >= 5 && $data->is_verificator != 1) text-gray-400 border-gray-400 @endif">
                                <input type="radio" name="verifikator" id="verifikator1"
                                    @if ($verifikator >= 5 && $data->is_verificator != 1) disabled @endif value="1"
                                    {{ $data->is_verificator ? 'checked' : '' }}>
                                <label for="verifikator1">
                                    Aktif
                                </label>
                            </div>
                            <div>
                                <input type="radio" name="verifikator" id="verifikatorx" value="0"
                                    {{ $data->is_verificator ? '' : 'checked' }}>
                                <label for="verifikatorx">Nonaktif
                                </label>
                            </div>
                        </div>
                    </div>
