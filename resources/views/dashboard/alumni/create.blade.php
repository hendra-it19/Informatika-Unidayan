        ...
        <div class="bg-white shadow rounded px-4 py-8">
            <form action="{{ route('alumni.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-3">
                    ...
                    <x-form.input type="number" name="nim" label='NIM' :value="old('nim')" />
                    <x-form.input type="text" name="tempat_lahir" label='Tempat Lahir' :value="old('tempat_lahir')" />
                    <x-form.input type="date" name="tanggal_lahir" label='Tanggal Lahir' :value="old('tanggal_lahir')" />
                    <x-form.input type="number" name="tahun_masuk" label='Tahun Masuk' :value="old('tahun_masuk')" />
                    <x-form.select name="status_masuk" label='Status Masuk' placeholder='Pilih Status Masuk'>
                        @foreach ($status_masuk as $r)
                            @if ($r['nama'] == old('status_masuk'))
                    ...
                    <x-form.input type="number" name="tahun_lulus" label='Tahun Lulus' :value="old('tahun_lulus')" />
                    <x-form.input type="text" name="ipk" label='IPK' :value="old('ipk')" />
