...
@section('pages')
    ...
    <section class="min-h-screen">
        ...
        <div class="px-4 py-6 shadow bg-white rounded">
            @if ($data_tunggu)
                <a class="btn-secondary cursor-not-allowed">Belum dikonfirmasi</a>
            @elseif($data_terima >= 2)
                <a class="btn-secondary cursor-not-allowed">Maksimal 2 Kegiatan</a>
            @else
                <a href="{{ route('kampus-merdeka.daftar') }}" class="btn-primary">Daftar Kegiatan</a>
            @endif
                    ...
                    <tbody>
                        @forelse ($data as $r)
                            <tr class="bg-white border-b hover:bg-primary-50 text-xs">
                                ...
                                <td class="p-2 lg:p-3">
                                    {{ $r->dosenPembimbing->nama ?? 'Belum ditetapkan' }}
                                </td>
                                <td class="p-2 lg:p-3">
                                ...
                                <td class="p-2 lg:p-3">
                                    <div class="flex gap-1.5 flex-col text-center">
                                        <a href="{{ asset($r->bukti_penerimaan) }}" target="_blank"
                                            class="py-0.5 px-3 rounded duration-300 bg-primary-500 hover:bg-primary-600 focus:bg-primary-700 focus:ring-2 focus:ring-primary-100 text-white">Penerimaan
                                            Mitra</a>
                                        <a href="{{ asset($r->persetujuan_kampus) }}" target="_blank"
                                            class="py-0.5 px-3 rounded duration-300 bg-primary-500 hover:bg-primary-600 focus:bg-primary-700 focus:ring-2 focus:ring-primary-100 text-white">Persetujuan
                                            Kampus</a>
                                    </div>
                                </td>
                                <td class="p-2 lg:p-3">
                                ...
                                <td class="p-2 lg:p-3 relative">
                                    <div class="flex flex-col gap-1">
                                        @switch($r->status)
                                            @case('menunggu')
                                                <form action="{{ route('kampus-merdeka.batal-daftar', $r->id) }}" method="POST">
                                                    @csrf
                                                    @method('delete')
                                            ...
                                            @case('diterima')
                                                <a href="{{ route('kampus-merdeka.detail', $r->id) }}"
                                            ...
                                            @case('ditolak')
                                                @if (!$data_tunggu && $data_terima < 2)
                                                    <a href="{{ route('kampus-merdeka.daftar') }}"
