...
@section('pages')
    <section class="min-h-screen">
        ...
        <div class="bg-white shadow rounded px-4 py-8">
            <form action="{{ route('akun-organisasi.store') }}" method="POST" enctype="multipart/form-data">
                @csrf

                <div class="mb-3 aspect-video max-h-64 bg-gray-100 rounded shadow-sm overflow-hidden">
                    <img src="https://placehold.co/1920x1080?text=16:9" alt="preview" id="foto-preview"
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-3 mb-3">
                    ...
                    <div class="flex gap-2 w-full items-center">
                        <div class="w-full">
                            <label class="label-ct" for="logo">Logo (Ratio 1:1) <span
                            ...
                            <input
                                ...
                                input-file
                            @enderror"
                                id="logo" name="logo" type="file" required accept=".svg,.png,.jpg,.jpeg,.webp">
                            @error('logo')
                                <small class="-mt-[1px] block text-red-500">{{ $message }}</small>
                            @enderror
                        </div>
                        <div class="w-[130px] relative">
                            <img src="https://placehold.co/400x400?logo" alt="logo"
                                class="w-[100px] absolute -bottom-[58px]" id="logo-preview">
                        </div>
                    </div>
                    <x-form.input type="text" name="nama_organisasi" label='Nama Organisasi' :value="old('nama_organisasi')" />
                    <x-form.input type="text" name="nama_penanggung_jawab" label='Nama Penanggung Jawab'
                        :value="old('nama_penanggung_jawab')" />
                    <x-form.input type="text" name="username" label='Username' :value="old('username')" />
                    <x-form.input type="text" name="password" label='Password' :value="old('password')" />
                    <div>
                        <label for="postingan" class="text-gray-800">Membuat Postingan</label>
                        <div class="flex gap-3">
                            <div>
                                <input type="radio" name="postingan" id="postingan1" value="1"
                                    {{ old('postingan') == 1 ? 'checked' : '' }}>
                                <label for="postingan1">Beri Akses</label>
                            </div>
                            <div>
                                <input type="radio" name="postingan" id="postingan2" value="0"
                                    {{ old('postingan') == 0 ? 'checked' : '' }}>
                                <label for="postingan2">Cegah Akses</label>
                            </div>
                        </div>
                    </div>
                </div>
                <x-form.text-editor type="text" name="keterangan_tambahan" label='Keterangan Tambahan' :value="old('keterangan_tambahan')"
                    :required="true" />
                <div class="mt-7 flex
                        gap-4 items-center">
