...
<li>
    <button type="button" class="w-full @if (request()->routeIs(['kerja-praktek.*', 'admin-msib.*'])) sidebar-active @else sidebar @endif"
    ...
    <ul id="dropdown-kp-msib" class="hidden py-1 space-y-0.5">
        <li>
            <a href="{{ route('admin-kp.index') }}"
                class="@if (request()->routeIs(['admin-kp.*'])) sidebar-child-active
        @else sidebar-child @endif">Kerja
                Praktek</a>
        </li>
        <li>
            <a href="{{ route('admin-msib.index') }}"
                class="@if (request()->routeIs(['admin-msib.*'])) sidebar-child-active
